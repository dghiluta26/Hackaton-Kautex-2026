...
def render_report_tab() -> None:
    ...
    if topic_costs:
        ...
        # Apply clean background gradients to help stakeholders instantly scan cost intensities
        styled_cost_df = df_display.style.background_gradient(
            cmap="Blues",
            subset=["Internal", "External", "Total"]
        ).format({
            "Internal": "${:,.2f}",
            "External": "${:,.2f}",
            "Recovery": "${:,.2f}",
